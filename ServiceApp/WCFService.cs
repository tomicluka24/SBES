...
namespace ServiceApp
{
    public class WCFService : IWCFContract
    {
        ...
        {
            if (Thread.CurrentPrincipal.IsInRole("heatCtrl") || Thread.CurrentPrincipal.IsInRole("humidityCtrl")
                || Thread.CurrentPrincipal.IsInRole("pressureCtrl") || Thread.CurrentPrincipal.IsInRole("windCtrl"))
                Console.WriteLine("Communication established.");
            else
            {
                string name = Thread.CurrentPrincipal.Identity.Name;
                throw new FaultException($"[TestCommunication] Exception. User {name} tried to call TestCommunication(). Access is denied.");
            }
        }
        ...
        public void SendMessage(string message, byte[] sign)
        {
            ...
            {
                string name = Thread.CurrentPrincipal.Identity.Name;
                throw new FaultException($"[SendMessage] Exception. User {name} tried to call SendMessage(). Access is denied.");
            }
